    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('auto-indent.fix', async () => {
            ...
            try {
                await normalizeWhitespace(editor);
                await fixBracePositions(editor);
                await convertVariableNamesToPascalCase(editor);
                await reindentCode(editor);
                vscode.window.showInformationMessage('Indentation and naming fixed successfully');
            } catch (error) {
                vscode.window.showErrorMessage('Failed to fix indentation: ' + error);
            }
        })
...
async function convertVariableNamesToPascalCase(editor: vscode.TextEditor) {
    const doc = editor.document;
    const variableRegex = /\b(?:int|float|double|char|bool|string)\s+([a-zA-Z_][a-zA-Z0-9_]*)/g;
    
    await editor.edit(editBuilder => {
        for (let line = 0; line < doc.lineCount; line++) {
            const textLine = doc.lineAt(line);
            let updatedText = textLine.text;
            
            updatedText = updatedText.replace(variableRegex, (match, varName) => {
                const pascalCaseName = toPascalCaseWithDictionary(varName);
                return match.replace(varName, pascalCaseName);
            });
            
            if (updatedText !== textLine.text) {
                editBuilder.replace(textLine.range, updatedText);
            }
        }
    });
...
function toPascalCaseWithDictionary(text: string): string {
    const tokenizer = new natural.WordTokenizer();
    const words = tokenizer.tokenize(text.replace(/_/g, ' '));
